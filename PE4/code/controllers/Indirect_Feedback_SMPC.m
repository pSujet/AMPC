...
classdef Indirect_Feedback_SMPC < Controller
    ...
    methods
        function obj = Indirect_Feedback_SMPC(sys, params, K, P, solver)
            ...
            z = sdpvar(sys.n,obj.params.N+1);
            e_bar = sdpvar(sys.n,obj.params.N+1);
            x_k = sdpvar(sys.n,1); % true state x(k); input to the optimizer
            z_1 = sdpvar(sys.n,1); % last predicted nominal state z_1|k-1; input to the optimizer
            v = sdpvar(sys.m,obj.params.N);
            ...
            constraints = [x_bar(:,1) == x_k];
            constraints = [constraints, z(:,1) == z_1];
            for i = 1:obj.params.N
                ...
                constraints = [constraints, sys.X.A*z(:,i) <= sys.X.b - xtight(:,i)];
                constraints = [constraints, sys.U.A*v(:,i) <= sys.U.b - utight(:,i)];
            end
            constraints = [constraints, z(:,obj.params.N+1) == [0;0]];
